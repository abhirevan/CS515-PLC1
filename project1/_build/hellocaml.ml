...
let fourthpower : int -> int = 
   ...
   *)
   ...
   let fourthpower (x: int) : int  = x * cube x;;
...
let cents_of (q:int) (d:int) (n:int) (p:int) : int = q * 25 + d *10 + n * 5 + p;;
...
let rec helper (l1: 'a list) (l2 : 'b list) =
begin match l1 with 
	| [] -> l2
	| h :: tl -> helper tl ( h :: l2 )
end

let rev_t (l: 'a list) : 'a list = helper l []
...
let rec length_list (l: 'a list) : int =
begin match l with
	| [] -> 0
	| h :: t -> 1 + length_list t
end
...
let rec left_list (l: 'a list) (m: int) : 'a list = 
if m = 0 
	then []
else  match l with 
		| [] -> []
		| h::t -> h :: left_list t (m-1)

let rec right_list (l: 'a list) (m: int) : 'a list =
if m = 0 
	then l
else match l with 
		| [] -> []
		| h::t -> right_list t (m-1)
	
let rec merge (a: 'a list) (b: 'a list) = 
...
let rec msort (l: 'a list): 'a list  = 
	if length_list l > 1 
	then merge (msort (left_list l (length_list l/2))) (msort (right_list l (length_list l/2)))
	else l
...
begin match e with 
	| Var v-> lookup v c
	| Const con -> con
	| Add(left,right) -> Int32.add (interpret c left) (interpret c right)
	| Mult(left,right) -> Int32.mul (interpret c left) (interpret c right)
	| Neg n -> Int32.neg (interpret c n)
end 
...
begin match e with 
	| Var v-> Var v
	| Const con -> Const con
	| Add(left,right) -> 	begin match left,right with 	| left, Const 0l -> left | Const 0l,right -> right | Const a, Const b -> Const (Int32.add a b) 
											| Var v, Const c-> Add(Var v, Const c) | Const c,Var v ->  Add( Const c,Var v ) 
											| Var m,Var n ->  Add(Var m,Var n ) 
											| _, Neg (Const 0l) -> left | Neg (Const 0l), _ -> right											
											|_,_ -> match optimize left,optimize right with | Const a, Const b -> optimize (Add(optimize left,optimize right) )  
																				 |_ , Neg (Const 0l) -> optimize (Add(optimize left,optimize right) )  | Neg (Const 0l), _ -> optimize (Add(optimize left,optimize right) ) 
																				 | _ , _ -> Add(optimize left,optimize right)
					end
	| Mult(left,right) -> 	begin match left,right with 	| _, Const 0l -> Const 0l | Const 0l, _ -> Const 0l | Const 1l,right -> right | left, Const 1l -> left | Const a, Const b -> Const (Int32.mul a b) 
											| Var v, Const c-> Mult(Var v, Const c) | Const c,Var v ->  Mult( Const c,Var v ) 
											| Var m,Var n ->  Mult(Var m,Var n )  
											| _, Neg (Const 0l) -> Const 0l | Neg (Const 0l ), _ -> Const 0l 
											|_,_ -> match optimize left,optimize right with | Const a, Const b -> optimize (Mult(optimize left,optimize right) )  
																				|_ , Neg (Const 0l) -> optimize (Mult(optimize left,optimize right) )  | Neg (Const 0l), _ -> optimize (Mult(optimize left,optimize right) )
																				| _ , _ -> Mult(optimize left,optimize right)
					end
	| Neg n -> Neg (optimize n)
end
...
begin match e with
		| Var v -> [IPushV v]
		| Const c -> [IPushC c]
		| Add(left, right) -> compile left @ compile right @ [IAdd]
		|Mult(left,right) -> compile left @ compile right @ [IMul]
...
begin match e with 
	...
	| Add(left,right) -> 	begin match left,right with 	| left, Const 0l -> left | Const 0l,right -> right | Const a, Const b -> Const (Int32.add a b) 
											| Var v, Const c-> Add(Var v, Const c) | Const c,Var v ->  Add( Const c,Var v ) 
											| Var m,Var n ->  Add(Var m,Var n )  
											|_,_ -> match optimize left,optimize right with | Const a, Const b -> optimize (Add(optimize left,optimize right) )  | _ , _ -> Add(optimize left,optimize right)
					end
	| Mult(left,right) -> 	begin match left,right with 	| _, Const 0l -> Const 0l | Const 0l, _ -> Const 0l | Const 1l,right -> right | left, Const 1l -> left | Const a, Const b -> Const (Int32.mul a b) 
											| Var v, Const c-> Mult(Var v, Const c) | Const c,Var v ->  Mult( Const c,Var v ) 
											| Var m,Var n ->  Mult(Var m,Var n )  
											|_,_ -> match optimize left,optimize right with | Const a, Const b -> optimize (Mult(optimize left,optimize right) )  | _ , _ -> Mult(optimize left,optimize right)
					end
	| Neg n -> Neg (optimize n)
end
*)
